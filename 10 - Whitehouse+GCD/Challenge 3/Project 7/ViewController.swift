...
class ViewController: UITableViewController {
  ...
  func filter(alert: UIAlertController?) {
    DispatchQueue.global().async { [weak self, weak alert] in
      guard let filterKey = alert?.textFields?[0].text?.lowercased() else { return }
